...
class PageClass extends mixins(PagePropertiesMixin, Container) {
  readonly state = {
  ...
  get childPages(): VNode[] {
    ...
    return Object.keys(this.cqChildren).map<VNode>((itemKey) => {
      ...
      const ItemComponent: any = this.state.componentMapping.get(
        itemProps.cqType
      );
...
export const Page = toNative(PageClass);
